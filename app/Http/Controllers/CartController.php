...
class CartController extends Controller
{
    ...
    public function removeItem($id)
    {
        ...
        $response = $this->requestConnection(
            'order/remove?order_hash='.session('hash')['order_hash'].'&order_group_id='.$id.'&paid_monthly_invoice='.session('paid_monthly_invoice'),
            'patch'
        );
        ...
        return $this->successRedirect(
            '/#devices-section',
            'The <span style="color: #6004ba;">'.session('deleted_item').'</span> was removed successfully.'
        );
    }
