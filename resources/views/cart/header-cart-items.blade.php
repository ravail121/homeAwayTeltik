<div class="col-md-2 col-sm-12 col-xs-12 pull-right header_cart">
	<div id="cart-drop">
		<a href="#" class="btn style1 btn-cart dropdown-icon">
                ...
                @if (sprintf("%.2f", $subtotalPrice) != 0)
                    ...
                    @if (session('cart')['active_group_id'] == 0)
                        @if (session('cart')['business_verification'] != null)
                            @if (!session('id'))
                                <a href="{{ url('checkout?verification_hash='.session('cart')['business_verification']['hash'].'&order_hash='.session('cart')['order_hash']) }}"
                                   class="btn system-background">Place Order</a>
                            @else
                                <a href="{{ url('checkout?order_hash='.session('cart')['order_hash']) }}"
                                   class="btn system-background">Place Order</a>
                            @endif
                        @else
                            <a href="{{ route('verify-bussiness.index') }}" class="btn system-background"
                              >Place Order</a>

                        @endif
                    @else
                        <a href="#cart-popup" data-toggle="modal" class="btn active-order-group system-background">
                            Place Order</a>

                    @endif
                @else
                    <a href="#cart-popup" data-toggle="modal" class="btn place-order system-background">
                        Place Order</a>
                @endif
            @endisset
        </div>
...
<style>
    ...
    .dropdown-icon::before {
        border-top-color: var(--system-color) !important;
    }
</style>
