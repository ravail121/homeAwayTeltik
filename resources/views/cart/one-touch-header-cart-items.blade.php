...
{{--		</a>--}}
		<div class="drop-con">
            <strong class="selected-item-text">Selected Items</strong>
			<ul class="cart-list">
                @if(session('cart'))
                    @foreach(session('cart')['order_groups'] as $cart)
                        <li style="background-color: {{ ($activeGroupId == $cart['id']) ? '#fff8d3' : '' }};">
                            @if ($cart['device'] != null)
                                <div class="img-wrap">
                                    <img src="{{ $cart['device']['primary_image'] ?: asset('imgs/placeholders/default.png') }}" alt="" class="{{ $cart['device']['primary_image'] }}">
                                </div>
                            ...
                            <div class="info {{ ($cart['device'] === 0 || ($cart['device'] === null && $cart['plan'] != null)) ? 'override-width' : ''}}">
                                <table>
                                    <tr>
                                        @if($cart['device'] != null)
                                            <td>{{-- Device: --}}  <a
                                                    href="javascript:void;{{-- route('devices.index') --}}"
                                                    class="item-title modify-device system-text"
                                                    {{-- data-order_group_id="{{ $cart['id'] }}" data-active_group_id="{{ $activeGroupId }}" data-device_id="{{ $cart['device']['id'] }}" --}}>
                                                    {{ $cart['device']['name'] }}
                                                    {{-- <span class="tooltiptext">Edit Device</span> --}}
                                                </a></td>
                                        ...
                                        @elseif ($cart['device'] === 0)
                                            <td>{{-- Device:  --}}<a href="javascript:void;" class="item-title system-text">Bringing Own Device</a></td>
                                            <td>--</td>
                                    ...
                                    <tr>
                                        @isset($cart['plan'])
                                            <td>{{-- Plan: --}} <a href="javascript:void;"
                                                                   class="item-title modify-plan system-text"
                                                    {{--  data-type="plan" data-active_group_id="{{ $activeGroupId }}" data-order_group_id="{{ $cart['id'] }}" --}}>
                                                    {{  $cart['plan']['name'] }}
                                    ...
                                    @if($cart['sim'] != null)
                                        @if($cart['plan'] != null)
                                            <tr>
                                                <td><a href="javascript:void;" class="item-title system-text">{{-- Sim Card: --}} {{ $cart['sim']['name'] }}</a></td>
                                                <td>$
                                    ...
                                    @elseif ($cart['sim_num'] != null)
                                        <tr>
                                           {{--  <td>Sim Number: <a href="#" class="item-title modify-sim" data-type="sim" data-active_group_id="{{ $activeGroupId }}" data-order_group_id="{{ $cart['id'] }}">{{ $cart['sim_num'] }}<span class="tooltiptext">Edit Number</span></a></td> --}}
                                            <td><a href="javascript:void;" class="item-title modify-sim system-text">
                                                    {{ $cart['sim_type'] }}
                                                </a>
                                                <p class='sim_num_{{$cart["id"]}} sim-number'>
                                                    SIM: {{ $cart['sim_num'] }}
                                                </p></td>
                                            <td></td>
                                        </tr>
                                        ...
                                        @foreach ($cart['addons'] as $addon)
                                            <tr>
                                                <td>{{-- {{ $loop->iteration }} Add-On: --}}
                                                    <a href="javascript:void;" class="item-title modify-addon system-text" {{-- data-type="addon" data-active_group_id="{{ $activeGroupId }}" data-order_group_id="{{ $cart['id'] }}" --}}>{{ $addon['name'] }}{{-- <span class="tooltiptext">Edit Addon</span> --}}</a>
                                    ...
                                    @isset($cart['plan'])
                                        @if ($cart['plan']['amount_onetime'] > 0)
                                            <td>
                                                <a href="javascript:void;" class="item-title system-text">
                                                    Activation Fee
                                                </a>
                                            </td>
                                    ...
                                    <div class="text-center">
                                        @if (session('id') || isset(session('cart')['business_verification']) && session('cart')['business_verification'] == null)
                                            <a href="{{ route('cart.destroy', $cart['id']) }}"
                                               ...
                                               data-confirm="Are you sure?" class='remove-cartItem system-text'
                                            >
                ...
                @if (sprintf("%.2f", $subtotalPrice) != 0)
                    ...
                    @if (session('cart')['active_group_id'] == 0)
                        @if (session('cart')['business_verification'] != null)
                            ...
                            @else
                                <a href="{{ url('checkout?order_hash='.session('cart')['order_hash']) }}" class="btn">
                                    Place Order
                                </a>
                            @endif
                        @else
                            <a href="{{ route('verify-bussiness.index') }}" class="btn">
                                Place Order
                            </a>
                        @endif
                    @else
