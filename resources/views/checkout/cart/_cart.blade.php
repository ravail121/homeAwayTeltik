<div class="checkout-item-con">
				...
				<li style="background-color: {{ ($activeGroupId == $cart['id']) ? '#fff8d3' : '' }};">
                    ...
                    <div class="info {{ ($cart['device'] === 0 || ($cart['device'] === null && $cart['plan'] != null)) ? 'override-width' : ''}}">
                        <table>
                            <tr>
                                @if($cart['device'] != null)
                                    <td style='text-align:left' class="td-label">
                                        Device:
                                        <a href="javascript:void;route('devices.index')"
                                           class="item-title modify-device"
                                            {{-- data-order_group_id="{{ f$cart['id'] }}" data-active_group_id="{{ $activeGroupId }}" data-device_id="{{ $cart['device']['id'] }}" --}}>{{ $cart['device']['name'] }}{{-- <span class="tooltiptext">Edit Device</span> --}}</a></td>

                                    <td style='width: 140px' class="td-value"> $
                                        @if ($cart['plan'] == null)
                                ...
                                @elseif ($cart['device'] === 0)
                                    <td style='text-align:left' class="td-label">
                                        {{-- Device:  --}}
                                        <a href="javascript:void;" class="item-title">Bringing Own Device</a>
                                    </td>
                            ...
                            <tr>
                                @isset($cart['plan'])
                                    <td style='text-align:left;' class="td-label">{{-- Plan: --}}
                            ...
                            @elseif ($cart['sim_num'] != null)
                                <tr>
                                   {{--  <td>Sim Number: <a href="#" class="item-title modify-sim" data-type="sim" data-active_group_id="{{ $activeGroupId }}" data-order_group_id="{{ $cart['id'] }}">{{ $cart['sim_num'] }}<span class="tooltiptext">Edit Number</span></a></td> --}}
                                    <td style='text-align:left' class="td-label">
                                        <a href="javascript:void;" class="item-title modify-sim">
                                            {{ $cart['sim_type'] }}
                                        </a>
                                        <p class='sim_num_{{$cart["id"]}}' style="font-size: 12px">
                                            SIM: {{ $cart['sim_num'] }}
                                        </p></td>
                                    <td class="td-value">--</td>
                                </tr>
                            ...
                            @if($cart['addons'])
                                @foreach ($cart['addons'] as $addon)
                                    <tr style='white-space: nowrap;'>
                                        <td style='text-align:left' class="td-label">{{-- {{ $loop->iteration }} Add-On: --}}
                            ...
                            @isset($cart['plan'])
                                @if ($cart['plan']['amount_onetime'] > 0)
                                    ...
                                    <td class="td-value">$ @convert($cart['plan']['amount_onetime'] )</td>
                                @endif
                            @endisset
                        </table>
                    </div>
	...
	<div class="summary">
        <table>
            ...
            <tr>
                <td class="td-label">Shipping Fee:</td>
                <td>+ $
            ...
            <tr>
                <td class="td-label">State Tax:</td>
                <td>+ $
                    <span class='tax-cart'>
            ...
            <tr>
                <td class="td-label">Regulatory Fee:</td>
                <td>+ $
