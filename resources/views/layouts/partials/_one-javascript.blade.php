{!! Html::script('https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js') !!}
{!! Html::script('js/vendors/jquery-ui.min.js') !!}
{!! Html::script('plugins/jquery-validator/jquery.validate.js') !!}
{!! Html::script('plugins/inputmask/jquery.inputmask.bundle.js') !!}
{!! Html::script('https://cdn.jsdelivr.net/jquery.validation/1.16.0/additional-methods.min.js') !!}
{!! Html::script('theme/js/jquery.marquee.min.js') !!}
{{--{!! Html::script('theme/js/bootstrap.min.js') !!}--}}
{!! Html::script('theme/js/functions.min.js') !!}
{!! Html::script('theme/js/main.js') !!}
{!! Html::script('js/extra-main.js') !!}
{!! Html::script('js/laravel.js') !!}
{!! Html::script('js/dropzone.js') !!}

{!! Html::script('https://unpkg.com/sweetalert/dist/sweetalert.min.js') !!}

{!! Html::script('js/vendors/jquery.dataTables.min.js') !!}
{!! Html::script('js/vendors/dataTables.bootstrap.min.js') !!}

<!-- JavaScript -->
<script src="{{ asset('js/vendors/flickity.pkgd.min.js') }}"></script>
<!-- Template Main JS File -->
{!! Html::script('theme/newstyle/js/main.js') !!}
